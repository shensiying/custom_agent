...
from custom_agent.single_agent.database import get_order, get_orders, update_order, order_exists
import requests
from custom_agent.single_agent.skills_loader import match_skill, list_skills_brief
...
def select_skill(need_description: str) -> str:
    """
    根据用户需求描述，从技能库中匹配最合适的业务处理技能（SOP）。
    必须在处理用户具体请求之前调用此工具获取标准操作流程，然后严格按流程执行。
    参数 need_description 为用户需求的简要概括，如 "用户要退货" 或 "用户要修改地址"。
    """
    skill = match_skill(need_description)
    if skill is None:
        available = list_skills_brief()
        return (
            f"未找到与「{need_description}」完全匹配的技能。\n\n"
            f"当前可用技能：\n{available}\n\n"
            "请根据用户需求选择最接近的技能，或按通用客服流程处理。"
        )
    return (
        f"技能名称: {skill['name']}\n"
